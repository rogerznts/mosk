...
Loaded eagerly by hallmark.md Step 3 whenever the catalog pick is `carnival`. The default palette ("Cold Snap") and font stack live in [`tokens.css`](tokens.css) under `[data-theme="carnival"]`; this file carries the **palette drops** (six variants) plus the rules that tokens cannot encode.
