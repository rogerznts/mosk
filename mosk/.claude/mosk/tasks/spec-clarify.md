...
1. Run `.claude/mosk/scripts/check-prerequisites.sh --json --paths-only` from repo root **once** (combined `--json --paths-only` mode). Parse minimal JSON payload fields:
   - `FEATURE_DIR`
   - `FEATURE_SPEC`
...
4. Sequential questioning loop (interactive):
    ...
    - For multiple-choice questions:
       - **Analyze all options** and determine the **most suitable option** based on best practices, risk reduction, and alignment with project goals.
