...
Loaded eagerly by hallmark.md Step 3 whenever the catalog pick is `lumen`. Palettes + font stack live in [`tokens.css`](tokens.css) under `[data-theme="lumen"]` (Night, default) and `[data-theme="lumen"][data-drop="day"]` (Day). This file carries the drops + apparatus family + signature moves + motion + anti-patterns.
