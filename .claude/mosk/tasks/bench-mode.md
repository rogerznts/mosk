...
- **Pós-condição:** escrever artefatos, confirmar somente as fases canônicas da
  spec via `transition-spec-phase.sh` e fazer **append** em `decisions-log.md`
  quando houve auto-escalação (ADR-0002). `build-loop` e `deliver` são estados
  internos do Bench e nunca são gravados em `spec-meta.yaml.current_phase`.
