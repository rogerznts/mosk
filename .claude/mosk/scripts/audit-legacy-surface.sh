...
if [ -d "$ROOT/.claude" ]; then
    ...
    while IFS=: read -r file line rest; do
        [ -n "$file" ] || continue
        rel="${file#"$ROOT/"}"
        # `.claude/rules/` é contexto do projeto consumidor, gerado por `boot`
        # a partir do código dele — não é superfície distribuível do toolkit.
        # Um projeto que de fato usa a ferramenta legada tem o direito de dizer
        # isso na própria rule; cobrar ali seria cobrar o usuário pelo
        # vocabulário do projeto dele, não limpar o MOSK.
        case "$rel" in .claude/rules/*) continue ;; esac
